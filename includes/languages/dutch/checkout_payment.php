<?php
/*
  $Id$

  CE Phoenix, E-Commerce made Easy
  https://phoenixcart.org

  Copyright (c) 2022 Phoenix Cart 

  Released under the GNU General Public License
*/

const NAVBAR_TITLE_1 = 'Afrekenen';
const NAVBAR_TITLE_2 = 'Betaling door u';

const HEADING_TITLE = 'Betaling door u';

const TABLE_HEADING_BILLING_ADDRESS = 'Facturatie adres';

const TABLE_HEADING_PAYMENT_METHOD = 'Betaalmethode';

const TEXT_ENTER_PAYMENT_INFORMATION = 'Dit is momenteel de enige betalingsmethode die beschikbaar is voor deze bestelling.';

const BUTTON_CONTINUE_CHECKOUT_PROCEDURE = 'Ga door met afrekenen';
